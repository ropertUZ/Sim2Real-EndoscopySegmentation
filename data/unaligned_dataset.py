...
class UnalignedDataset(BaseDataset):
    ...
    def __init__(self, opt):
        ...
        print(opt.phase)
        if opt.phase == 'train':
            self.dir_B_gt = os.path.join(opt.dataroot,opt.phase + 'B_depth') 
    ...
    def __getitem__(self, index):
        ...
        if self.opt.phase == 'train':
            B_gt_img = Image.open(B_gt_path).convert('RGB')
        #B_gt_img = B_gt_img.resize((256,256))
        #print(np.unique(B_gt_img))
        ...
        if self.opt.phase == 'train':
            self.transform_B_gt = get_transform(self.opt, params_B, grayscale=True, binary=False)#True)
            B_gt_img = self.transform_B_gt(B_gt_img)
        

        A = self.transform_A(A_img)
        B = self.transform_B(B_img)
        #print(torch.min(A), torch.max(A))
        #B_gt = torch.from_numpy(np.asarray(B_gt_img).astype(np.uint8)) #self.transform_B_gt(B_gt_img)
        if self.opt.phase == 'train':
            return {'A': A, 'B': B, 'B_gt' : B_gt_img, 'A_paths': A_path, 'B_paths': B_path}
        else:
