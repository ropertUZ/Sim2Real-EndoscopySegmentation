...
def get_transform(opt, params=None, grayscale=False, method=Image.BICUBIC, convert=True, binary=False):
    transform_list = []
    if binary:
...
def __flip(img, flip):
    if flip:
        return img.transpose(Image.FLIP_LEFT_RIGHT).transpose(Image.FLIP_TOP_BOTTOM).transpose(Image.ROTATE_90)
    return img
